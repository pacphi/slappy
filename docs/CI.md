...
- Strict Vue and CLI typechecks are blocking. c8 enforces 80% statements, branches, functions, and lines across executable TypeScript; Vue templates are outside this percentage. A separate Vitest gate enforces the same thresholds for all `app/components/**/*.vue` files. Coverage and complexity reports are uploaded as CI artifacts.
- `pnpm test:web` starts the built server, checks HTML/PDF APIs, and exercises stock search, keyboard selection, and quoted CSV upload through the browser UI. Axe scans the homepage, upload, mapping, and preview states; violations fail the job, and accessibility results are uploaded.
